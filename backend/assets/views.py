...
class RBACPermission(permissions.BasePermission):
    ...
    Roles: super_admin, inventory_manager, staff, auditor
    """
    ...
    def has_permission(self, request, view):
        ...
        if method in ['GET', 'HEAD', 'OPTIONS']:
            ...
            if action_name == 'checkout' or action_name == 'checkin':
                required = {'transfer'}
...
class AssetViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        log_audit_action(
            self.request, 'create', 'Asset', instance.id, str(instance),
    ...
    def checkout(self, request, pk=None):
        ...
        if user.role not in ['super_admin', 'inventory_manager']:
            return Response(
                {'error': 'Only Inventory Manager can checkout assets'},
        ...
        with transaction.atomic():
            ...
            TransferHistory.objects.create(
                ...
                reason=request.data.get('reason', 'Asset checkout'),
                performed_by=user
            )
        
        log_audit_action(
            request, 'checkout', 'Asset', asset.id, str(asset),
            {'assigned_to': asset.assigned_to_id, 'status': asset.status}
        )
    ...
    def checkin(self, request, pk=None):
        ...
        if user.role not in ['super_admin', 'inventory_manager']:
            return Response(
                {'error': 'Only Inventory Manager can checkin assets'},
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        with transaction.atomic():
            ...
            TransferHistory.objects.create(
                ...
                reason=request.data.get('reason', 'Asset checkin'),
                performed_by=user
            )
        
        log_audit_action(
            request, 'checkin', 'Asset', asset.id, str(asset),
    ...
    def transfer(self, request, pk=None):
        ...
        if user.role not in ['super_admin', 'inventory_manager']:
            return Response(
                {'error': 'Only Inventory Manager can transfer assets'},
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        with transaction.atomic():
            ...
            TransferHistory.objects.create(
                ...
                reason=request.data.get('reason', 'Asset transfer'),
                performed_by=user
            )
        
        log_audit_action(
            request, 'transfer', 'Asset', asset.id, str(asset),
            {
    ...
    def report_damage(self, request, pk=None):
        ...
        TransferHistory.objects.create(
            asset=asset,
            transfer_type='maintenance',
            reason=request.data.get('description', 'Damage reported'),
            performed_by=request.user
        )
        
        log_audit_action(
            request, 'report_damage', 'Asset', asset.id, str(asset),
            {'status': {'old': old_status, 'new': 'in_repair'}}
        )
...
class TransferHistoryViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        log_audit_action(
            self.request, 'transfer', 'TransferHistory', 
            serializer.instance.id if serializer.instance else None,
            str(serializer.instance) if serializer.instance else ''
        )
...
class AssetTypeViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        serializer.save()
        log_audit_action(
            self.request, 'create', 'AssetType',
            serializer.instance.id, str(serializer.instance),
            {'name': serializer.instance.name, 'code': serializer.instance.code}
        )
    ...
    def perform_update(self, serializer):
        ...
        serializer.save()
        log_audit_action(
            self.request, 'update', 'AssetType',
            old_instance.id, str(old_instance),
            {'name': {'old': old_instance.name, 'new': serializer.instance.name}}
        )

    def perform_destroy(self, instance):
        log_audit_action(
            self.request, 'delete', 'AssetType',
            instance.id, str(instance),
            {'name': instance.name}
        )
        instance.delete()